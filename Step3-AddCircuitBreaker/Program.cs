...
// TASK 2: Combine retry + circuit breaker
...
// TASK 3: Update /predict endpoint to use combined policy
app.MapGet("/predict/{threshold:double}", (double threshold, ObservationStore observationStore, ModelService modelService) =>
{
    using var activity = activitySource.StartActivity("PredictWithCircuitBreaker");
    activity?.SetTag("threshold", threshold);

    Log.Information("Starting prediction with circuit breaker + retry for threshold {Threshold}", threshold);

    try
    {
        // TASK 3: Replace retryPolicy with combinedPolicy
    ...
    {
        Log.Error(ex, "Prediction failed after all retries and circuit breaker");
